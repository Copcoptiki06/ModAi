  ...
  return <main className="teacher-shell admin-shell">
    ...
    <div className="admin-layout">
      ...
      <section className="teacher-main">
        ...
        {showEditor&&<article className="admin-card question-editor standalone-editor"><div className="section-head"><div><h2>Soru bilgileri</h2><p>Soruyu tamamlayıp öğrenci havuzuna kaydedin.</p></div><select value={status} onChange={e=>setStatus(e.target.value)}><option value="published">Yayında</option><option value="draft">Taslak</option></select></div><img src="/question-images/refraction-air-glass.png" alt="Havadan cama geçen ışığın normale yaklaşarak kırılması"/><label>Soru türü<select value={type} onChange={e=>setType(e.target.value)}><option value="multiple_choice">Çoktan seçmeli</option><option value="true_false">Doğru / Yanlış</option><option value="open_ended">Açık uçlu</option></select></label><label>Soru metni<textarea value={prompt} onChange={e=>setPrompt(e.target.value)} /></label><div className="option-grid"><button>A — Normale yaklaşır</button><button>B — Normalden uzaklaşır</button><button>C — Yön değiştirmez</button><button>D — Geri yansır</button></div><div className="editor-actions"><button className="secondary" onClick={()=>setShowEditor(false)}>Vazgeç</button><button onClick={saveQuestion}>Soruyu kaydet</button></div></article>}
        ...
        {!showEditor&&section==="visuals"&&<article className="admin-card section-card"><div className="section-head"><div><h2>Fen görsel kütüphanesi</h2><p>Sorularda kullanılabilen özgün kavram görselleri</p></div><button className="primary-small" onClick={()=>setNotice("Yeni görseller ünite paketleri halinde eklenecek.")}>+ Görsel ekle</button></div><div className="visual-grid"><button onClick={()=>{setSection("questions");setShowEditor(true);}}><img src="/question-images/refraction-air-glass.png" alt="Işığın kırılması"/><b>Işığın kırılması</b><span>7. sınıf • Kullan</span></button></div></article>}
