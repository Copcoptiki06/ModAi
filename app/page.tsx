...
function ModelLaboratory(){
  ...
  const refracted=Math.round(Math.asin(Math.sin(angle*Math.PI/180)/(medium==="cam"?1.5:medium==="su"?1.33:1))*180/Math.PI),incomingRotation=270-angle,outgoingRotation=90-refracted,rayLength=180,sourceX=50+Math.cos(incomingRotation*Math.PI/180)*rayLength/4,sourceY=50+Math.sin(incomingRotation*Math.PI/180)*rayLength/2.5,energy=Math.round(mass*9.8*height),mixtureAnswer=mixture==="kum-su"?"Süzme":mixture==="tuz-su"?"Buharlaştırma":"Mıknatıs";
