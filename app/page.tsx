...
function AdminDashboard({ onLogout }: { onLogout: () => void }) {
  const [data,setData]=useState<{users:Array<{id:number;role:string}>;classes:Array<{id:number}>;questions:Array<Record<string,unknown>>;attempts:Array<Record<string,unknown>>;aiInteractions:Array<Record<string,unknown>>}>({users:[],classes:[],questions:[],attempts:[],aiInteractions:[]});
  ...
  useEffect(()=>{load();},[]);
  async function saveQuestion(){setNotice("Kaydediliyor…");const response=await fetch("/api/data",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"create_question",grade:7,unit:"Işığın Kırılması",topic:"Işığın kırılması",type,prompt,options:["Normale yaklaşır","Normalden uzaklaşır","Yön değiştirmez","Geri yansır"],correctAnswer:"Normale yaklaşır",hint:"İkinci ortamın optik yoğunluğunu ve ışığın hızını karşılaştır.",imageUrl:"/question-images/refraction-air-glass.png",status})});const result=await response.json() as {error?:string};setNotice(response.ok?"Soru kaydedildi ve öğrencilere hazır.":result.error||"Kaydedilemedi.");if(response.ok)load();}
  ...
  return <main className="teacher-shell admin-shell">
    ...
    <div className="admin-layout">
      ...
      <section className="teacher-main">
        <div className="teacher-title"><div><p>MODAI YÖNETİM MERKEZİ</p><h1>Tüm sistem tek ekranda</h1><span>Öğretmen, öğrenci, içerik ve yapay zekâ etkileşimlerini izle ve yönet.</span></div><button onClick={saveQuestion}>+ Soruyu kaydet</button></div>
        ...
        <div className="admin-grid">
          <article className="admin-card question-editor"><div className="section-head"><div><h2>Görselli soru düzenleyici</h2><p>Yapay zekâ bu sorunun içeriğine göre ipucu üretir.</p></div><select value={status} onChange={e=>setStatus(e.target.value)}><option value="published">Yayında</option><option value="draft">Taslak</option></select></div><img src="/question-images/refraction-air-glass.png" alt="Havadan cama geçen ışığın normale yaklaşarak kırılması"/><label>Soru türü<select value={type} onChange={e=>setType(e.target.value)}><option value="multiple_choice">Çoktan seçmeli</option><option value="true_false">Doğru / Yanlış</option><option value="open_ended">Açık uçlu</option></select></label><label>Soru metni<textarea value={prompt} onChange={e=>setPrompt(e.target.value)} /></label><div className="option-grid"><button>A — Normale yaklaşır</button><button>B — Normalden uzaklaşır</button><button>C — Yön değiştirmez</button><button>D — Geri yansır</button></div><div className="ai-hint"><b>ModAi ipucu</b><p>Önce ikinci ortamın optik yoğunluğunu karşılaştır. Işık daha yoğun ortama geçerken hızının nasıl değiştiğini düşün.</p><button onClick={()=>setNotice("İpucu soru bağlamına göre hazırlandı.")}>İpucunu yeniden üret</button></div></article>
          <article className="admin-card"><div className="section-head"><div><h2>Soru bankası</h2><p>Tüm sınıf ve ünitelerin merkezi içerik alanı</p></div><button className="filter-button">7. sınıf</button></div><div className="question-list">{displayQuestions.map((item,index) => <div key={`${item.title}-${index}`}><span className="question-type">{item.type}</span><p><b>{item.title}</b><small>{item.grade} • {item.unit}{item.visual ? " • Görselli" : ""}</small></p><strong className={item.status === "Yayında" ? "published" : "draft"}>{item.status}</strong><button onClick={()=>setPrompt(item.title)}>Düzenle</button></div>)}</div><div className="curriculum-box"><b>Ortaokul fen görsel kütüphanesi</b><p>5, 6, 7 ve 8. sınıf kavram görselleri ünite bazında eklenebilir. İlk paket: 7. sınıf Işığın Kırılması.</p><span>%12 ilk paket hazır</span></div></article>
        </div>
