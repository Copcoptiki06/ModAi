...
const grade7Labs = [
  {id:"telescope",unit:"Uzay Çağı",title:"Teleskopu ayarla",goal:"Objektif ve göz merceği odaklarını değiştirerek büyütmeyi incele."},
  {id:"pendulum",unit:"Kuvvet ve Enerji",title:"Sarkaç enerjisi",goal:"Potansiyel ve kinetik enerji dönüşümünü hareket boyunca gözlemle."},
  {id:"meiosis",unit:"Hücre ve Bölünmeler",title:"Mayoz modeli",goal:"Mayoz sonunda kromozom sayısının yarıya inmesini modelle."},
  {id:"pollination",unit:"Canlılarda Üreme, Büyüme ve Gelişme",title:"Tozlaşma ve döllenme",goal:"Polenin taşınmasından tohum oluşumuna kadar süreci yönet."},
  {id:"recycling",unit:"Saf Madde ve Karışımlar",title:"Atıkları ayrıştır",goal:"Atıkları özelliklerine göre doğru geri dönüşüm kutusuna yönlendir."},
  {id:"atom",unit:"Saf Madde ve Karışımlar",title:"Atomu kur",goal:"Proton, nötron ve elektron sayılarını değiştirerek atom modelini incele."},
  {id:"lens",unit:"Işığın Madde ile Etkileşimi",title:"Mercekte odak",goal:"Mercek türü ve cisim uzaklığının ışınlara etkisini keşfet."},
  {id:"kinetic",unit:"Kuvvet ve Enerji",title:"Kinetik enerji",goal:"Kütle ve hızın hareket enerjisine etkisini karşılaştır."},
  {id:"weight",unit:"Kuvvet ve Enerji",title:"Gezegenlerde ağırlık",goal:"Kütle değişmeden ağırlığın gök cismine göre değişimini modelle."},
  {id:"lifecycle",unit:"Canlılarda Üreme, Büyüme ve Gelişme",title:"Yaşam döngüsü",goal:"Başkalaşım aşamalarını sıralı model üzerinde incele."},
  {id:"debris",unit:"Uzay Çağı",title:"Uzay kirliliğini temizle",goal:"Yörüngedeki atık yoğunluğu ve çarpışma riskini modelle."},
  ...
  {id:"work",unit:"Kuvvet ve Enerji",title:"İş hesapla",goal:"Kuvvet ve yol değiştikçe yapılan işi karşılaştır."},
  {id:"circuit",unit:"Elektrik Devreleri",title:"Devreyi çalıştır",goal:"Anahtar ve ampul sayısının devreye etkisini keşfet."},
  {id:"circuit-compare",unit:"Elektrik Devreleri",title:"Seri–paralel karşılaştır",goal:"Bağlantı biçiminin parlaklık ve devre sürekliliğine etkisini keşfet."},
  ...
  {id:"solution",unit:"Saf Madde ve Karışımlar",title:"Çözelti derişimi",goal:"Çözünen ve çözücü miktarını değiştirerek derişimi modelle."},
  {id:"mirror",unit:"Işığın Madde ile Etkileşimi",title:"Aynalarda görüntü",goal:"Ayna türü ve cisim uzaklığının görüntüye etkisini incele."},
...
function ModelLaboratory(){
  ...
  const refracted=Math.round(Math.asin(Math.sin(angle*Math.PI/180)/(medium==="cam"?1.5:medium==="su"?1.33:1))*180/Math.PI),incomingRotation=270-angle,outgoingRotation=90-refracted,rayLength=120,sourceOffsetX=Math.cos(incomingRotation*Math.PI/180)*rayLength,sourceOffsetY=Math.sin(incomingRotation*Math.PI/180)*rayLength,energy=Math.round(mass*9.8*height),mixtureAnswer=mixture==="kum-su"?"Süzme":mixture==="tuz-su"?"Buharlaştırma":"Mıknatıs";
  ...
  return <section className="model-labs" id="model-labs"><div className="lab-heading"><div><p className="eyebrow">7. SINIF MODEL LABORATUVARI</p><h2>Değişkenleri yönet, tahmin et ve sonucu gözle</h2><p>Her modelde önce bir tahmin yap; sonra değişkenleri değiştirerek düşünceni kanıtla.</p></div><span>{grade7Labs.length} etkileşimli model</span></div><div className="lab-tabs">{grade7Labs.map(item=><button key={item.id} className={lab===item.id?"active":""} onClick={()=>setLab(item.id)}><small>{item.unit}</small><b>{item.title}</b></button>)}</div><div className="lab-workbench"><div className="lab-goal"><span>MODEL GÖREVİ</span><b>{grade7Labs.find(item=>item.id===lab)?.goal}</b></div>
    ...
    {lab==="debris"&&<div className="lab-grid"><div className="lab-controls"><label>Atık yoğunluğu <b>%{debris}</b><input type="range" min="10" max="100" value={debris} onChange={e=>setDebris(Number(e.target.value))}/></label><label>Temizleme kapasitesi <b>%{cleaning}</b><input type="range" min="0" max="100" value={cleaning} onChange={e=>setCleaning(Number(e.target.value))}/></label><p>Çarpışma riski %{Math.max(0,debris-cleaning)}. {cleaning>=debris?"Yörünge güvenli düzeye indi.":"Temizleme kapasitesini artır veya yeni atık oluşumunu azalt."}</p></div><div className="debris-sim"><span className="earth-mini">Dünya</span>{Array.from({length:Math.ceil(debris/10)}).map((_,i)=><i key={i} style={{transform:`rotate(${i*37}deg) translateX(${75+i%3*24}px)`}}/>)}<b style={{width:`${cleaning}%`}}>temizleme</b></div></div>}
    {lab==="work"&&<div className="lab-grid"><div className="lab-controls"><label>Kuvvet <b>{force} N</b><input type="range" min="5" max="100" value={force} onChange={e=>setForce(Number(e.target.value))}/></label><label>Alınan yol <b>{distance} m</b><input type="range" min="1" max="20" value={distance} onChange={e=>setDistance(Number(e.target.value))}/></label><p>Yapılan iş: <strong>{force*distance} joule</strong>. Kuvvet veya yol arttığında yapılan iş artar.</p></div><div className="work-sim"><span className="box" style={{left:`${8+distance*3.5}%`}}>Kutu</span><i style={{width:`${Math.min(80,force*.7)}%`}}/><b>{force} N →</b><em>{distance} m</em></div></div>}
    {lab==="circuit-compare"&&<div className="lab-grid"><div className="lab-controls"><label>Bağlantı türü<select value={connection} onChange={e=>setConnection(e.target.value)}><option value="seri">Seri bağlantı</option><option value="paralel">Paralel bağlantı</option></select></label><p>{connection==="seri"?"Ampuller akımı paylaşır; biri çıkarılırsa devre kesilir.":"Ampuller ayrı kollardadır; biri çıkarılsa diğer kol çalışmaya devam eder."}</p></div><div className={`compare-circuit ${connection}`}><span className="battery">+ −</span><i/><div><b>●</b><b>●</b></div><em>{connection==="seri"?"Daha sönük":"Daha parlak"}</em></div></div>}
    {lab==="solution"&&<div className="lab-grid"><div className="lab-controls"><label>Çözünen madde <b>{solute} g</b><input type="range" min="5" max="100" value={solute} onChange={e=>setSolute(Number(e.target.value))}/></label><label>Su <b>{solvent} mL</b><input type="range" min="50" max="250" value={solvent} onChange={e=>setSolvent(Number(e.target.value))}/></label><p>Derişim göstergesi %{Math.min(100,Math.round(solute/solvent*180))}. {solute/solvent>.45?"Derişik çözelti":"Seyreltik çözelti"} oluştu.</p></div><div className="solution-sim"><div className="solution-glass" style={{background:`rgba(219,113,54,${Math.min(.85,.12+solute/solvent)})`}}>{Array.from({length:Math.ceil(solute/10)}).map((_,i)=><i key={i}/>)}</div><b>{solute/solvent>.45?"Derişik":"Seyreltik"}</b></div></div>}
    {lab==="mirror"&&<div className="lab-grid"><div className="lab-controls"><label>Ayna türü<select value={mirror} onChange={e=>setMirror(e.target.value)}><option value="duz">Düz ayna</option><option value="cuk">Çukur ayna</option><option value="tum">Tümsek ayna</option></select></label><label>Cisim uzaklığı <b>{objectDistance} cm</b><input type="range" min="15" max="100" value={objectDistance} onChange={e=>setObjectDistance(Number(e.target.value))}/></label><p>{mirror==="duz"?"Görüntü cisimle aynı boyda ve aynanın arkasında oluşur.":mirror==="tum"?"Görüntü her zaman küçük ve düzdür.":objectDistance<35?"Cisim odağın içinde: büyük ve düz görüntü.":"Cisim odağın dışında: ters görüntü."}</p></div><div className={`mirror-sim ${mirror}`}><span className="mirror-object" style={{left:`${Math.max(5,38-objectDistance/4)}%`}}>↑</span><i/><span className="mirror-image" style={{transform:`scale(${mirror==="tum"?.55:mirror==="cuk"&&objectDistance<35?1.5:1}) ${mirror==="cuk"&&objectDistance>=35?-1:1})`}}>↑</span></div></div>}
    {lab==="atom"&&<div className="lab-grid"><div className="lab-controls"><label>Proton <b>{protons}</b><input type="range" min="1" max="12" value={protons} onChange={e=>setProtons(Number(e.target.value))}/></label><label>Nötron <b>{neutrons}</b><input type="range" min="0" max="14" value={neutrons} onChange={e=>setNeutrons(Number(e.target.value))}/></label><label>Elektron <b>{electrons}</b><input type="range" min="1" max="12" value={electrons} onChange={e=>setElectrons(Number(e.target.value))}/></label><p>Atom numarası {protons}; kütle numarası {protons+neutrons}. {protons===electrons?"Atom nötrdür.":protons>electrons?"Pozitif iyon oluştu.":"Negatif iyon oluştu."}</p></div><div className="atom-sim"><span className="nucleus">{Array.from({length:Math.min(12,protons+neutrons)}).map((_,i)=><i className={i<protons?"p":"n"} key={i}/>)}</span><b className="electron-ring">{Array.from({length:Math.min(8,electrons)}).map((_,i)=><em key={i} style={{transform:`rotate(${i*45}deg) translateX(88px)`}}/>)}</b><small>{protons}p + {neutrons}n + {electrons}e</small></div></div>}
    {lab==="lens"&&<div className="lab-grid"><div className="lab-controls"><label>Mercek türü<select value={lens} onChange={e=>setLens(e.target.value)}><option value="ince">İnce kenarlı</option><option value="kalin">Kalın kenarlı</option></select></label><label>Cisim uzaklığı <b>{lensDistance} cm</b><input type="range" min="20" max="100" value={lensDistance} onChange={e=>setLensDistance(Number(e.target.value))}/></label><p>{lens==="ince"?(lensDistance<40?"Cisim odağa yakın: büyük ve sanal görüntü.":"Işınlar odakta toplanır; gerçek görüntü oluşabilir."):"Işınlar dağılarak ilerler; küçük ve sanal görüntü oluşur."}</p></div><div className={`lens-sim ${lens}`}><span className="lens-object">↑</span><i className="lens-shape"/><b/><em/><span className="lens-focus">F</span></div></div>}
    {lab==="kinetic"&&<div className="lab-grid"><div className="lab-controls"><label>Kütle <b>{movingMass} kg</b><input type="range" min="1" max="10" value={movingMass} onChange={e=>setMovingMass(Number(e.target.value))}/></label><label>Hız <b>{speed} m/s</b><input type="range" min="1" max="15" value={speed} onChange={e=>setSpeed(Number(e.target.value))}/></label><p>Kinetik enerji yaklaşık <strong>{Math.round(movingMass*speed*speed/2)} joule</strong>. Hız iki katına çıkınca enerji dört katına çıkar.</p></div><div className="kinetic-sim"><span className="moving-ball" style={{width:`${28+movingMass*3}px`,height:`${28+movingMass*3}px`,left:`${8+speed*4}%`}}/><i>{"›".repeat(Math.ceil(speed/3))}</i><div><b style={{width:`${Math.min(100,movingMass*speed*speed/18)}%`}}/></div></div></div>}
    {lab==="weight"&&<div className="lab-grid"><div className="lab-controls"><label>Gök cismi<select value={planet} onChange={e=>setPlanet(e.target.value)}><option>Dünya</option><option>Ay</option><option>Mars</option><option>Jüpiter</option></select></label><label>Kütle <b>{bodyMass} kg</b><input type="range" min="10" max="100" value={bodyMass} onChange={e=>setBodyMass(Number(e.target.value))}/></label><p>Kütle her yerde {bodyMass} kg kalır. {planet} üzerindeki ağırlık yaklaşık <strong>{Math.round(bodyMass*({Dünya:9.8,Ay:1.62,Mars:3.71,Jüpiter:24.79}[planet]||9.8))} N</strong>.</p></div><div className={`weight-sim ${planet.toLocaleLowerCase("tr-TR")}`}><span>{planet}</span><i style={{transform:`scale(${.65+bodyMass/220})`}}>●</i><b>↓ yer çekimi</b></div></div>}
    {lab==="lifecycle"&&<div className="lab-grid"><div className="lab-controls"><label>Yaşam aşaması <b>{lifeStep+1}/4</b><input type="range" min="0" max="3" value={lifeStep} onChange={e=>setLifeStep(Number(e.target.value))}/></label><p>{["Yumurtalar suya bırakılır.","İribaş solungaçlarıyla solunum yapar.","Bacaklar gelişir, kuyruk küçülür.","Ergin kurbağa yaşam döngüsünü sürdürür."][lifeStep]}</p></div><div className={`life-sim life-${lifeStep}`}><div>{lifeStep===0?"○ ○ ○":lifeStep===1?"●〰":lifeStep===2?"●╱╲〰":"🐸"}</div><span>{["Yumurta","İribaş","Genç kurbağa","Ergin kurbağa"][lifeStep]}</span></div></div>}
    {lab==="telescope"&&<div className="lab-grid"><div className="lab-controls"><label>Objektif odak uzaklığı <b>{objective} cm</b><input type="range" min="40" max="140" value={objective} onChange={e=>setObjective(Number(e.target.value))}/></label><label>Göz merceği odak uzaklığı <b>{eyepiece} cm</b><input type="range" min="5" max="25" value={eyepiece} onChange={e=>setEyepiece(Number(e.target.value))}/></label><p>Teleskop yaklaşık <strong>{Math.round(objective/eyepiece)} kat</strong> büyütür. Objektif odağı artıp göz merceği odağı azaldıkça büyütme artar.</p></div><div className="telescope-sim"><div className="scope"><i/><b/></div><span className="space-target" style={{transform:`scale(${.55+objective/eyepiece/18})`}}>✦</span><em>{Math.round(objective/eyepiece)}×</em></div></div>}
    {lab==="pendulum"&&<div className="lab-grid"><div className="lab-controls"><label>Sarkacın konumu <b>{pendulum}°</b><input type="range" min="-70" max="70" value={pendulum} onChange={e=>setPendulum(Number(e.target.value))}/></label><p>Potansiyel enerji %{Math.round(Math.abs(pendulum)/70*100)}, kinetik enerji %{100-Math.round(Math.abs(pendulum)/70*100)}. Toplam enerji yaklaşık korunur.</p></div><div className="pendulum-sim"><i style={{transform:`rotate(${pendulum}deg)`}}><b/></i><div className="energy-split"><span style={{width:`${Math.round(Math.abs(pendulum)/70*100)}%`}}>P</span><em>K</em></div></div></div>}
    {lab==="meiosis"&&<div className="lab-grid"><div className="lab-controls"><label>Mayoz aşaması <b>{meiosisStep+1}/4</b><input type="range" min="0" max="3" value={meiosisStep} onChange={e=>setMeiosisStep(Number(e.target.value))}/></label><p>{["Kromozomlar eşlenir ve homolog çiftler yan yana gelir.","Homolog kromozomlar ayrılır; iki hücre oluşur.","Kardeş kromatitler ikinci kez ayrılır.","Kromozom sayısı yarıya inmiş dört üreme hücresi oluşur."][meiosisStep]}</p></div><div className={`meiosis-sim meiosis-${meiosisStep}`}>{Array.from({length:meiosisStep===3?4:meiosisStep>0?2:1}).map((_,i)=><div key={i}><b>×</b><b>{meiosisStep<2?"×":"│"}</b></div>)}<span>{meiosisStep===3?"2n → n":"Kromozomların ayrılması"}</span></div></div>}
    {lab==="pollination"&&<div className="lab-grid"><div className="lab-controls"><label>Süreç aşaması <b>{pollinationStep+1}/4</b><input type="range" min="0" max="3" value={pollinationStep} onChange={e=>setPollinationStep(Number(e.target.value))}/></label><p>{["Polen başçıkta oluşur.","Polen böcek veya rüzgârla dişicik tepesine taşınır.","Polen tüpü yumurtalığa ulaşır ve döllenme gerçekleşir.","Tohum ve meyve gelişmeye başlar."][pollinationStep]}</p></div><div className={`pollination-sim pollination-${pollinationStep}`}><span className="flower">✿</span><i className="pollen">•</i><b className="bee">◆</b><em>{pollinationStep===3?"Tohum oluştu":"Polenin yolculuğu"}</em></div></div>}
    {lab==="recycling"&&<div className="lab-grid"><div className="lab-controls"><label>Atık türü<select value={waste} onChange={e=>{setWaste(e.target.value);setWasteBin("");}}><option value="cam">Cam şişe</option><option value="kagit">Kâğıt kutu</option><option value="plastik">Plastik ambalaj</option><option value="pil">Atık pil</option></select></label><div className="method-buttons">{["cam","kagit","plastik","pil"].map(item=><button key={item} onClick={()=>setWasteBin(item)}>{item.toLocaleUpperCase("tr-TR")}</button>)}</div><p>{!wasteBin?"Uygun geri dönüşüm kutusunu seç.":wasteBin===waste?"Doğru kutu: madde geri kazanım sürecine girebilir.":"Bu kutu uygun değil; atığın ham maddesini yeniden düşün."}</p></div><div className="recycling-sim"><span className={`waste-item ${waste}`}>{waste==="cam"?"♙":waste==="kagit"?"▤":waste==="plastik"?"▱":"▥"}</span><div>{["cam","kagit","plastik","pil"].map(item=><b className={wasteBin===item?"selected":""} key={item}>{item}</b>)}</div></div></div>}
  </div></section>;
}
